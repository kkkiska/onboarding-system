...
const Login = () => {
  ...
  const submitForm = async (data) => {
    ...
    if (LoginCache.isCached(data.email, data.password)) {
      setError({
        message: "Неверные данные уже были отправлены ранее",
        code: "Cached",
      });
      setLoading(false);
      return;
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.error || "Произошла ошибка при входе";
  ...
  const isButtonDisabled =
    ...
    errors.email ||
    loading ||
    error.code.includes(ATTEMPTS_ERROR);
    ...
    <div className={styles["login"]}>
      <Plate className={styles["login__plate"]}>
        <img
          ...
          src={logo}
          alt="Логотип NTEC - onboarding system"
        />
        <div className={styles["login__welcome"]}>
          <h1 className={styles["login__welcome-title"]}>Добро пожаловать!</h1>
          <p className={styles["login__welcome-subtitle"]}>
            Вход в онбординговую систему
          </p>
        </div>
        <form
        ...
        >
          ...
          {showTimer && (
            <LoginLimitTimer initialTime={60} onTimerEnd={handleTimerEnd} />
          )}
